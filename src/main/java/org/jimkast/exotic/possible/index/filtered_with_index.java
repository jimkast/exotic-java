...
public final class filtered_with_index<T> implements possible<T> {
    private final bicheck<T, Integer> check;
    private final with_index<T> origin;

    public filtered_with_index(bicheck<T, Integer> check, possible<T> origin) {
        this(check, new with_index<>(origin));
    }

    public filtered_with_index(bicheck<T, Integer> check, with_index<T> origin) {
        this.check = check;
        this.origin = origin;
    }
