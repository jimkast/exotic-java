...
public final class gen_until<T> implements possible<T> {
    private final check<T> check;
    private final UncheckedScalar<T> factory;

    public gen_until(check<T> check, Scalar<T> factory) {
        this(check, new UncheckedScalar<>(factory));
    }

    public gen_until(check<T> check, UncheckedScalar<T> factory) {
        this.check = check;
